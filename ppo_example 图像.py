...
# --- 配置参数 ---
DLL_PATH = r'c:\研究生\03设备信息\myRIO\DLL文件 - 参数\Dll1\x64\Debug\Dll1.dll'  # DLL文件路径
TOTAL_TRAINING_STEPS = 6000000  # 总训练步数
STEPS_PER_EPISODE = 2000  # 每个episode的步数
TIME_STEP = 0.01  # 时间步长
...
def run_single_environment(env_id, experience_queue, action_queues, state_queues, worker_args):
    ...
    while True:
        ...
        # 记录当前集数的角速度
        episode_angular_velocities.append(initial_angular_velocities)
        ...
        for step in range(steps_per_episode):
            ...
            next_input_array_sim = np.clip(action, [0, 0, 0, -5, -5, 0], [0, 0, 0, 5, 5, 0]).reshape(1, -1)
            ...
            if np.any(np.abs(current_output_array) >= max_output_value) or np.any(np.isnan(current_output_array)):
               ...
               #     print(f"[Worker {env_id}] Warning: 欧拉角未变化！Step {step}")  # 如果相同，打印警告信息
                ...
                euler_within_threshold = np.abs(euler_angles_current_sim) <= 1.0  # 检查欧拉角是否在阈值范围内
                angular_vel_within_threshold = np.abs(angular_velocities_current_sim) <= 30.0  # 检查角速度是否在阈值范围内

                if np.all(euler_within_threshold):  # 如果所有欧拉角都在阈值范围内
                    distances = 1.0 - np.abs(euler_angles_current_sim)  # 计算距离奖励
                    normalized_distances = distances / 1.0  # 归一化距离奖励
                    reward += np.sum(normalized_distances) * 100.0  # 将距离奖励添加到总奖励中

                if np.all(angular_vel_within_threshold):  # 如果所有角速度都在阈值范围内
                    reward += ANGULAR_VEL_STABLE_REWARD  # 将角速度稳定奖励添加到总奖励中

                time_reward = current_time_sim * 0.1  # 计算时间奖励
                reward += time_reward  # 将时间奖励添加到总奖励中

                excess_angles = np.abs(euler_angles_current_sim) - 1.0  # 计算超出角度阈值的部分
                excess_angles[excess_angles < 0] = 0  # 将负值设置为0
                if np.any(excess_angles > 0):  # 如果存在超出角度阈值的情况
                    reward -= np.sum(excess_angles) * 50.0  # 从总奖励中减去超出角度惩罚
